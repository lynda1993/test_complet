...
public class Main {
	...
	private static List<Transaction> loadTransactions() throws IOException {
		...
		for(String line : lines) {
			...
			transaction.setTxtId(Tab[0]);
			transaction.setDate(Tab[1]);
			transaction.setMagasin(Tab[2]);
			transaction.setIdProduit(Tab[3]);
			transaction.setQuantite(Integer.parseInt(Tab[4]));	
